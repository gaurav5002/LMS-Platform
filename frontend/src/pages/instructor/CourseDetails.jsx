...
import { ChevronDown, ChevronUp, Plus, Video, FileText, X, LogOut, Users, Clock, BookOpen } from 'lucide-react';
...
const CourseDetails = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#FFFDF6' }}>
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-2" style={{ color: '#2E4057' }}>Course Not Found</h2>
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8" style={{ backgroundColor: '#FFFDF6' }}>
      {/* Header */}
      ...
      {/* Course Overview */}
      <div className="bg-white rounded-xl shadow-sm overflow-hidden mb-8" style={{ backgroundColor: '#FAF6E9' }}>
      ...
      {/* Lessons List */}
      <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6" style={{ backgroundColor: '#FAF6E9' }}>
        <h2 className="text-xl sm:text-2xl font-bold mb-6" style={{ color: '#2E4057' }}>Lessons</h2>
        {lessons.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500">No lessons added yet. Add your first lesson to get started!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {lessons.map((lesson) => (
              <div key={lesson._id} className="border rounded-lg overflow-hidden" style={{ borderColor: '#DDEB9D' }}>
                <button
                  onClick={() => toggleLesson(lesson._id)}
                  className="w-full flex items-center justify-between p-4 hover:bg-[#DDEB9D] transition-colors"
                >
                  <div className="flex items-center">
                    <span className="font-medium mr-3" style={{ color: '#2E4057' }}>{lesson.title}</span>
                    <span className="text-sm text-gray-500">{lesson.duration || 0} min</span>
                  </div>
                  {expandedLessons[lesson._id] ? (
                    <ChevronUp className="w-5 h-5" style={{ color: '#2E4057' }} />
                  ) : (
                    <ChevronDown className="w-5 h-5" style={{ color: '#2E4057' }} />
                  )}
                </button>
                {expandedLessons[lesson._id] && (
                  <div className="p-4 border-t" style={{ borderColor: '#DDEB9D' }}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {lesson.videoUrl && (
                        <button
                          onClick={() => handleContentSelect(lesson._id, 'video')}
                          className={`flex items-center p-3 rounded-lg transition-colors ${
                            selectedContent?.lessonId === lesson._id && selectedContent?.type === 'video'
                      ...
                      {lesson.notesUrl && (
                        <button
                          onClick={() => handleContentSelect(lesson._id, 'file')}
                          className={`flex items-center p-3 rounded-lg transition-colors ${
                            selectedContent?.lessonId === lesson._id && selectedContent?.type === 'file'
                              ? 'bg-[#A0C878] text-white'
                              : 'hover:bg-[#DDEB9D] text-[#2E4057]'
                          }`}
                        >
